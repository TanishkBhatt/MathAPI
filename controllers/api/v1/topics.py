...
async def get_topics(database: AsyncIOMotorClient, api_key: str | None) -> Dict[str, Any]:
    await validate_api_key(database, api_key)
    
    # RETRIEVING ALL TOPICS
    try:
        topics: List[Dict[str, Any]] = await get_documents(database,"datasets","topics")
    ...
    for topic in topics:
        ...
        topic["sources_available"] = len(sources_data)
        topic["formulae_available"] = len(formulae_data)
